(function($) {
	...
	function coverSlider(){
		...
		$slider.each(function(){
			...
			if ( itemLength > 1 ){
				$this.prepend('<button type="button" class="prev">ì´ì „</button><button type="button" class="next">ë‹¤ìŒ</button>');
	...
	function viewMore(){
		// íŽ˜ì´ì§• ë°©ì‹ìœ¼ë¡œ í‘œì‹œ
		let show_paging = $("#tt-body-category").length > 0 || $("#tt-body-guestbook").length > 0;

		if ( $(".paging-view-more").length && $(".post-item").length && !show_paging ) {
			viewMoreShow();
		}
		...
		function viewMoreShow(){
			...
			$(".pagination a").hide();
			if( nextUrl ){
				$(".pagination").append('<a href="'+nextUrl+'" class="view-more">ëª©ë¡ ë”ë³´ê¸°</a>');
				$(".pagination .view-more").on("click", function(){
					window.dataLayer = window.dataLayer || [];
					dataLayer.push({'event':'vkf.view_more', 'vkf_url':decodeURIComponent(nextUrl)});

					viewMore(nextUrl);
					// íŽ˜ì´ì§€ ì´ë™ì´ ë˜ì§€ ì•Šë„ë¡ preventDefault ì²˜ë¦¬
					return false;
				});
			}
		}
		...
		function viewMore(url){
			$.ajax({
				...
				var $res = $(res),
						$nextPostItem = $res.find(".post-item:not(.post-item-manual)"),
